...
PATCHES_STACKS="$tmp/stacks" HOST="$host" PORT="$port" \
    node "$root/website/dist/server/entry.mjs" >"$tmp/server.log" 2>&1 &
server_pid=$!
...
# A stack added after the server starts must appear without rebuilding or
# restarting, which guards the runtime catalog behavior.
...
node -e '
  ...
  if (JSON.stringify(stacks) !== JSON.stringify(["demo", "runtime"])) process.exit(1);
